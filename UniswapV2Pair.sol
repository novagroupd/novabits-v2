...
contract UniswapV2Pair is IUniswapV2Pair, ERC20WithPermit {
    ...
    function _initialize(
        ...
        token0 = _token0;
        token1 = _token1;
        (bool success0, string memory symbol0) = MetadataHelper.getSymbol(
            _token0
        );
        (bool success1, string memory symbol1) = MetadataHelper.getSymbol(
            _token1
        );
        ...
        if (success0 && success1) {
            name_ = string(
                abi.encodePacked("ZkDex ", symbol0, "/", symbol1, " LP Token")
            );
